...
def get_pointclouds_boundaries(pc: Union[np.ndarray, torch.Tensor]):
    if isinstance(pc, np.ndarray):
        min_boundaries = np.min(pc, axis=0)
        max_boundaries = np.max(pc, axis=0)
    elif isinstance(pc, torch.Tensor):
        min_boundaries = torch.min(pc, axis=0).values
        max_boundaries = torch.max(pc, axis=0).values
    return min_boundaries, max_boundaries

def downsample_pointcloud(
    pc: Union[List, np.ndarray, torch.Tensor], 
    sample_num: int,
    shuffle: bool=False,
) -> Union[np.ndarray, torch.Tensor]:
    if isinstance(pc, List):
        pc = np.array(pc)
    assert pc.shape[0] >= sample_num, 'The number of point clouds is too small.'
    if isinstance(pc, np.ndarray):
        if shuffle:
            idx = np.random.permutation(pc.shape[0])
            pc = pc[idx[:sample_num]]
        else:
            pc = pc[:sample_num]
    elif isinstance(pc, torch.Tensor):
        if shuffle:
            idx = torch.randperm(pc.shape[0]).to(device=pc.device)
            pc = pc[idx[:sample_num]]
        else:
            pc = pc[:sample_num]
    return pc
